...
# save rules
sh -c 'iptables-save > /etc/firewall.conf'
sh -c 'echo "#!/bin/sh" > /etc/network/if-up.d/iptables'
sh -c 'echo "iptables-restore < /etc/firewall.conf" >> /etc/network/if-up.d/iptables'
